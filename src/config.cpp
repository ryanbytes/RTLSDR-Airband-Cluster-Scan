...
static const int DEFAULT_EDGE_GUARD_HZ = 150000;
static const int DEFAULT_CLUSTER_HANG_TIME_MS = 500;
...
static int parse_channels(libconfig::Setting& chans, device_t* dev, int i) {
    ...
    for (int j = 0; j < chans.getLength(); j++) {
        ...
        if (dev->mode == R_MULTICHANNEL || dev->mode == R_CLUSTER_SCAN) {
...
static void build_device_cluster_plan(device_t* dev, int devidx) {
    vector<ClusterScanFrequency> frequencies;
    for (int i = 0; i < dev->channel_count; i++) {
        ClusterScanFrequency frequency;
        frequency.frequency = dev->channels[i].freqlist[0].frequency;
        frequency.channel_index = i;
        frequencies.push_back(frequency);
    }

    try {
        ClusterScanPlan plan = build_cluster_scan_plan(frequencies,
                                                       dev->input->sample_rate,
                                                       dev->edge_guard_hz,
                                                       dev->max_cluster_width_hz,
                                                       dev->prefer_more_channels_per_cluster);
        dev->usable_bandwidth_hz = plan.usable_bandwidth_hz;
        dev->cluster_count = (int)plan.clusters.size();
        dev->clusters = (cluster_t*)XCALLOC(dev->cluster_count, sizeof(cluster_t));
        for (int i = 0; i < dev->cluster_count; i++) {
            cluster_t* cluster = dev->clusters + i;
            cluster->center_frequency = plan.clusters[(size_t)i].center_frequency;
            cluster->min_frequency = plan.clusters[(size_t)i].min_frequency;
            cluster->max_frequency = plan.clusters[(size_t)i].max_frequency;
            cluster->channel_count = (int)plan.clusters[(size_t)i].channel_indices.size();
            cluster->channel_indices = (int*)XCALLOC(cluster->channel_count, sizeof(int));
            for (int j = 0; j < cluster->channel_count; j++) {
                cluster->channel_indices[j] = plan.clusters[(size_t)i].channel_indices[(size_t)j];
            }
        }
    } catch (const invalid_argument& e) {
        cerr << "Configuration error: devices.[" << devidx << "]: " << e.what() << "\n";
        error();
    }

    if (dev->cluster_count < 1) {
        cerr << "Configuration error: devices.[" << devidx << "]: cluster_scan produced no clusters\n";
        error();
    }
    dev->current_cluster = 0;
    dev->input->centerfreq = dev->clusters[0].center_frequency;
}
    ...
    for (int i = 0; i < devs.getLength(); i++) {
        ...
        if (devs[i].exists("mode")) {
            if (!strncmp(devs[i]["mode"], "multichannel", 12)) {
                ...
                dev->mode = R_SCAN;
            } else if (!strncmp(devs[i]["mode"], "cluster_scan", 12)) {
                dev->mode = R_CLUSTER_SCAN;
            } else {
                cerr << "Configuration error: devices.[" << i << "]: invalid mode (must be one of: \"scan\", \"multichannel\", \"cluster_scan\")\n";
                error();
            }
        } else {
            dev->mode = R_MULTICHANNEL;
        }
        dev->edge_guard_hz = DEFAULT_EDGE_GUARD_HZ;
        dev->cluster_hang_time_ms = DEFAULT_CLUSTER_HANG_TIME_MS;
        dev->max_cluster_width_hz = 0;
        dev->prefer_more_channels_per_cluster = true;
        dev->cluster_scan_print_plan = false;
        dev->cluster_count = 0;
        dev->current_cluster = 0;
        dev->usable_bandwidth_hz = 0;
        dev->clusters = NULL;
        if (dev->mode == R_CLUSTER_SCAN) {
            if (devs[i].exists("edge_guard_hz")) {
                dev->edge_guard_hz = parse_anynum2int(devs[i]["edge_guard_hz"]);
            }
            if (devs[i].exists("cluster_hang_time_ms")) {
                dev->cluster_hang_time_ms = (int)devs[i]["cluster_hang_time_ms"];
            }
            if (devs[i].exists("max_cluster_width_hz")) {
                dev->max_cluster_width_hz = parse_anynum2int(devs[i]["max_cluster_width_hz"]);
            }
            if (devs[i].exists("prefer_more_channels_per_cluster")) {
                dev->prefer_more_channels_per_cluster = (bool)devs[i]["prefer_more_channels_per_cluster"];
            }
            if (devs[i].exists("cluster_scan_print_plan")) {
                dev->cluster_scan_print_plan = (bool)devs[i]["cluster_scan_print_plan"];
            }
            if (dev->edge_guard_hz < 0) {
                cerr << "Configuration error: devices.[" << i << "]: edge_guard_hz must not be negative\n";
                error();
            }
            if (dev->cluster_hang_time_ms < 0) {
                cerr << "Configuration error: devices.[" << i << "]: cluster_hang_time_ms must not be negative\n";
                error();
            }
            if (dev->max_cluster_width_hz < 0) {
                cerr << "Configuration error: devices.[" << i << "]: max_cluster_width_hz must not be negative\n";
                error();
            }
        }
        if (dev->mode == R_MULTICHANNEL) {
            dev->input->centerfreq = parse_anynum2int(devs[i]["centerfreq"]);
        }  // centerfreq for R_SCAN/R_CLUSTER_SCAN will be set after frequency list has been read
#ifdef NFM
        ...
        if (dev->mode == R_CLUSTER_SCAN) {
            build_device_cluster_plan(dev, i);
        }
