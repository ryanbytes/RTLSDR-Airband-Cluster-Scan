...
#define GOTOXY(x, y) printf("%c[%d;%dH", 0x1B, (y) + 1, (x) + 1)
...
struct freq_t {
    ...
    long long last_received_ms;    // last time this frequency had signal, unix epoch milliseconds
...
enum rec_modes { R_MULTICHANNEL, R_SCAN, R_CLUSTER_SCAN };
struct device_t {
    input_t* input;
#ifdef NFM
    float alpha;
...
extern bool print_cluster_scan_plan;
extern char* stats_filepath;
extern char* gui_status_filepath;
extern char* gui_last_received_filepath;
extern int gui_status_interval_ms;
